...
#ifndef __ALERTDLGLAYER__H__
#define __ALERTDLGLAYER__H__
...
class AlertDlg: public CCLayer {

public:
	AlertDlg();
	~AlertDlg();

	virtual bool init();CREATE_FUNC(AlertDlg)
	;
	...
	// 构架，并设置对话框背景图片
	static AlertDlg* create(const char* backgroundImage);

	// 它可以显示标题，并且设定显示文字大小
	void setTitle(const char*title, int fontsize = 20);
	// 文本内容，padding 为文字到对话框两边预留的距离，这是可控的，距上方的距离亦是如此
	void setContentText(const char *text, int fontsize = 20, int padding = 50, int paddintTop = 100, bool blink = false);
...
#endif /* defined(__ALERTDLGLAYER__H__) */
